  ...
  const handleAddMedia = async (e: React.FormEvent) => {
    ...
    try {
      if (newMediaType === 'photo') {
        if (!newMediaFile) {
          toast({ title: "File tidak boleh kosong", variant: "destructive" });
        ...
        if (result.error || !result.url) {
          throw new Error(result.error || 'Gagal mendapatkan URL setelah unggah.');
        }
        mediaUrlToSave = result.url;
      } else { // video
        if (!newMediaUrl) {
          ...
          <Dialog open={isAddMediaOpen} onOpenChange={setIsAddMediaOpen}>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleAddMedia} className="space-y-4">
                 ...
                 {newMediaType === 'photo' ? (
                   <div>
                     <Label htmlFor="media-file">File Foto</Label>
                     <Input
                       id="media-file"
                       type="file"
                       accept="image/*"
                       onChange={(e) => setNewMediaFile(e.target.files ? e.target.files[0] : null)}
                       required
                   ...
                   <div>
                     <Label htmlFor="media-url">URL Video (YouTube Embed)</Label>
                     <Input 
                       id="media-url" 
                       value={newMediaUrl} 
                 ...
                 <DialogFooter>
                   ...
                   <Button type="submit" disabled={isSubmitting}>
                     ...
                     {isSubmitting ? 'Mengunggah...' : 'Simpan Media'}
                   </Button>
                 </DialogFooter>
              </form>
            </DialogContent>
